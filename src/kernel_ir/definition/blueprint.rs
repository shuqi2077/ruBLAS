...
use crate::kernel_ir::{
    ...
    definition::{F32MathMode, MatmulElems, MatmulProblem, MatmulSetupError, TilingScheme},
...
pub fn adjust_dtypes<R: Runtime>(
    ...
    if requires_accelerator {
        if dtypes.f32_math == F32MathMode::AllowTf32
            && dtypes.lhs_global == f32_dtype
            && dtypes.rhs_global == f32_dtype
            && dtypes.acc_register == f32_dtype
