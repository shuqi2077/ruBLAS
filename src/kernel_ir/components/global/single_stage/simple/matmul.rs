    ...
    GW: GlobalWriter<MP::Acc>,
{
    ...
    fn execute(
        ...
        let num_loops = if comptime!(FIRST_K_TILE != 0) {
            let tile = FIRST_K_TILE.runtime();
            let remainder = range % tile;
            let first = u32::min(range, if remainder == 0 { tile } else { remainder });
            lhs_reader.set_first_k_step(first);
            rhs_reader.set_first_k_step(first);
            if range == 0 { 0u32.runtime() } else { 1 + (range - first).div_ceil(k_step) }
        } else {
            range.div_ceil(k_step)
        };
    ...
    fn init_lhs_global_reader(
        ...
        Self::LhsGlobalReader::new_with_checked_view(
            lhs,
            runtime_config,
    ...
    fn init_rhs_global_reader(
        ...
        Self::RhsGlobalReader::new_with_checked_view(
            rhs,
            runtime_config,
            config.stage_config.elements_in_stage_k(),
            config.rhs_reader_config,
