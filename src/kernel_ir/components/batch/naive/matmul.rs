...
impl<MT: MatmulTypes> BatchMatmul<(), MT> for NaiveMatmul<MT> {
    ...
    fn execute<Args: MatmulArgs>(
        ...
        let mut sequential = AccRE::<MT>::zero();
        ...
        for k in range_stepped(0u32, k, vector_size as u32) {
            ...
            let lhs = Vector::<AccRE<MT>, NA>::cast_from(lhs);
            let rhs = Vector::<AccRE<MT>, NA>::cast_from(rhs);
            if comptime!(config.sequential_accumulation) {
                #[unroll]
                for v in 0..vector_size {
                    sequential = fma(lhs[v], rhs[v], sequential);
                }
            } else if comptime!(config.fused_accumulation) {
                sum = fma(lhs, rhs, sum);
            } else {
                sum += lhs * rhs;
            }
        ...
        if comptime!(config.sequential_accumulation) {
            out[(m, n)] = Vector::cast_from(sequential);
        } else if unroll_sum {
