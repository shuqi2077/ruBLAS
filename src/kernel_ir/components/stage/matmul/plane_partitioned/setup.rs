...
use crate::kernel_ir::components::stage::matmul::plane_partitioned::{
...
type STy<T> = crate::kernel_ir::definition::Stage<T>;
type SSz<T> = crate::kernel_ir::definition::StageSize<T>;
    ...
    for PlaneMatmulFamily<StageLhs, StageRhs, StageAcc>
{
    ...
    fn rudadim_resource(
        ...
        if let RudaDimResource::Planes(planes) = blueprint.tile_matmul.rudadim_resource()? {
            Ok(RudaDimResource::Planes(
                planes
