...
use ruda_kernel::dsl::ir::features::TypeUsage;
...
use ruda_kernel::quantization::scheme::{QuantLevel, QuantMode, QuantParam, QuantScheme, QuantStore, QuantValue};
...
/// `ruda_kernel::quantization::quantize_native` currently requires for `QuantStore::Native`.
...
fn run_quantized_matmul(case: QuantizedMatmulCase) {
    ...
    let problem = MatmulProblem::from_parameters(
        ...
        ruda_kernel::dsl::ir::AddressType::U32,
    );
...
pub fn test_matmul_quantized_lhs_q8s_native() {
    // Native store = 1:1 packing. `ruda_kernel::quantization::quantize` routes this through
